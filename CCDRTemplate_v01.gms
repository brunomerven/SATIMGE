...
* for CCDR template
ReportE(CCDRIndicators,T,RUN)             Indicators for Template
REPORT_RUN(PRC,COM,T,Indicators) REPORT of indicators by run and process and commodity for each run
REPORTM(PRC,COM,T,RUN,Indicators) Merged Report
...
execute_load REPORT_RUN,GDP_RUN,POP,QGX,EGX,GSAVX,QVAX,YGX,MPSX,TINSX,YIX,PQHX,PQX,QHX,WFX,WFDISTX,QFSX,QFX,QEX,PEX,QMX,PMX,GVA_FS,EXRX;
...
ReportE('GDP',T,RUN) = GDP_RUN(T);

GDPperCapita(T)$POP(T) = GDP_RUN(T)/POP(T);
* note we're using GDP at factor costs real = GVA for GDP
LOOP(MY,
ReportE('GDPGrowth',MY,RUN)$GDP_RUN(MY-1) = ((GDP_RUN(MY)/GDP_RUN(MY-1))**(1/MYGAP(MY))-1)*100;
ReportE('GDPperCapitaGrowth',MY,RUN)$GDPperCapita(MY-1) = ((GDPperCapita(MY)/GDPperCapita(MY-1))**(1/MYGAP(MY))-1)*100;
*end of MY loop
);
* USD per capita
ReportE('GDPperCapita',T,RUN) = GDPperCapita(T)*1000000/EXR2019;
* USD per capita. Using 2019 prices for now. This may have to reviewed since prices are real throughout the period?
ReportE('HouseholdCons',T,RUN)$POP(T) = SUM((C,H),PQHX(C,H,'base','2019','2050')*QHX(C,H,'base',T,'2050'))/POP(T)*1000000/EXR2019;

ReportE('PrivateCons',T,RUN)$GDP_RUN(T) = SUM((C,H),PQHX(C,H,'base','2019','2050')*QHX(C,H,'base',T,'2050'))/GDP_RUN(T)*100;
ReportE('GovCons',T,RUN)$GDP_RUN(T) = SUM(C,PQX(C,'base','2019','2050')*QGX(C,'base',T,'2050'))/GDP_RUN(T)*100;

ReportE('PrivateInv',T,RUN)$GDP_RUN(T) = SUM(INSDNG,MPSX(INSDNG,'base',T,'2050')*(1-tinsx(INSDNG,'base',T,'2050'))*YIX(INSDNG,'base',T,'2050'))/GDP_RUN(T)*100;
ReportE('GovInv',T,RUN)$GDP_RUN(T) = GSAVX('base',T,'2050')/GDP_RUN(T)*100;

ReportE('NetExports',T,RUN)$GDP_RUN(T) = SUM(C,QEX(C,'rest','base',T,'2050')*PEX(C,'rest','base',T,'2050')-QMX(C,'rest','base',T,'2050')*PMX(C,'rest','base',T,'2050'))/GDP_RUN(T)*100;

ReportE('QVAAgri',T,RUN)$GDP_RUN(T) = GVA_FS('agr',T)/GDP_RUN(T)*100;
ReportE('QVAServices',T,RUN)$GDP_RUN(T) = GVA_FS('COM',T)/GDP_RUN(T)*100;
ReportE('QVAIndustry',T,RUN)$GDP_RUN(T) = (GDP_RUN(T)-GVA_FS('agr',T)-GVA_FS('COM',T))/GDP_RUN(T)*100;

ReportE('ExchangeRate',T,RUN) = EXRX('base',T,'2050');

ReportE('FiscalRevenue',T,RUN)$GDP_RUN(T) = YGX('base',T,'2050')/GDP_RUN(T)*100;
ReportE('FiscalExpenditure',T,RUN)$GDP_RUN(T) = EGX('base',T,'2050')/GDP_RUN(T)*100;

ReportE('BudgetDeficit',T,RUN)$GDP_RUN(T) = GSAVX('base',T,'2050')/GDP_RUN(T)*100;

ReportE('Emissions',T,RUN) = CO2eq(T,RUN);
ReportE('EmissionsperGDP',T,RUN)$GDP_RUN(T) = CO2eq(T,RUN)/GDP_RUN(T)*1000;


REPORTM(PRC,COM,T,RUN,Indicators) = REPORT_RUN(PRC,COM,T,Indicators);
* end of RUN loop
);
