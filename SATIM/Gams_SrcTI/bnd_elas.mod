...
*-----------------------------------------------------------------------------
  ...
  LOOP((ANNUAL(S),MI_DMAS(RD_AGG(R,C),COM)),
    ...
    MI_DOPE(R,T,COM)$MI_ESUB(R,T,C)=MAX(ROUND(ABS(COM_ELAST(R,T,COM,S,'FX')),5)-5E-6,MI_ESUB(R,T,C))$SUM(BDNEQ$COM_ELAST(R,T,C,S,BDNEQ),1));
...
* Price levels for CES (marginal / average)
  ...
  MI_AGC(R,T,COM,C,J,BD)$((COM_VOC(R,T,C,BD)>0)$MI_AGC(R,T,COM,C,J,BD)$MI_DOPE(R,T,C)) = BDSIG(BD)*
    (1-(1-BDSIG(BD)*ORD(J)*COM_VOC(R,T,C,BD)/COM_STEP(R,C,BD))**(1-1/MI_DOPE(R,T,C)))/(ORD(J)*COM_VOC(R,T,C,BD)/COM_STEP(R,C,BD))/(1-1/MI_DOPE(R,T,C));
