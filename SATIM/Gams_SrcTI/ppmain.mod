...
* Prepare demand sifting storages
  OPTION TRACKPC < STG_SIFT; TRACKPC(RP,C)$(NOT TOP(RP,C,'OUT')$PRC_TSL(RP,'DAYNITE')$RP_STG(RP))=NO;
  RPC_STGN(TOP(TRACKPC,'IN'))=YES; RPC_PKC(TRACKPC)=NO;
  ACT_TIME(RTP(R,T,P),BD)$(ACT_TIME(RTP,'FX')$RP_STG(R,P))=ACT_TIME(RTP,'FX')$BDUPX(BD);
  LOOP(TRACKPC(RP(R,P),COM),RP_STS(RP)=NO; RP_STG(RP)=NO; RHS_COMPRD(RTCS_VARC(R,T,COM,S))=YES;
    IF(SUM(RPC_STG(RP,C),1)>1,RPC_LS(RP,C(ACTCG))=YES; ELSE RPC_LS(RPC_STG(RP,C))=YES));
...
* FLO / IRE / COM
 ...
 UC_ATTR(R,UCN,SIDE,UC_GRPTYPE(COM_VAR+2),UC_DYNT)$UC_ATTR(R,UCN,SIDE,'COMCON',UC_DYNT)$=SUM(UC_GMAP_C(R,UCN,COM_VAR,C,'COMCON'),1);
