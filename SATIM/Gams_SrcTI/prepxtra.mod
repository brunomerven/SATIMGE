...
*-----------------------------------------------------------------------------
$ LABEL SAVE
  F=JNOW; Z=CARD("%GDXPATH%%RUN_NAME%")+1; MY_F=NOT execerror;
$IF %1==SAVE MODLYEAR(LL)=T(LL)+PASTYEAR(LL); PUT QLOG; FILE.PW=512;
$IF %G226%%1==YESSAVE
  PUT_UTILITY 'GDXOUT' / '%GDXPATH%%RUN_NAME%' @(Z+16) (GSECOND(F)+100):0:0 @(Z+14) (GMINUTE(F)+100):0:0 @(Z+12) (GHOUR(F)+100):0:0 @(Z+9) (GDAY(F)+100):0:0 "_" @(Z+7) (GMONTH(F)+100):0:0 @(Z+4) GYEAR(F):0:0 @Z '~Data_'; EXECUTE_UNLOAD;
  IF(execerror$MY_F,execerror=0);